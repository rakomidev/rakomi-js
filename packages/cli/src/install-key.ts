...
import { CI_FEDERATION_SESSION_CLIENT_ID, isCimdClientId } from './env.js';
...
/** Read-only: the ALREADY-PERSISTED install keypair for `clientId`, or `undefined` when absent,
 * corrupted, or bound to a DIFFERENT `clientId`. Never generates and never writes — a caller that
 * must not touch disk until some later condition holds (`runLoginCi` — see below) peeks first and
 * defers `persistInstallKey()` until that condition is met. `getOrCreateInstallKey()` is the
 * ordinary generate-and-persist-immediately convenience built on top of this. */
export function peekInstallKey(keys: KeyStore, clientId: string): StoredInstallKey | undefined {
...
/** Persist `key` under the one install-key `KeyStore` slot. */
export function persistInstallKey(keys: KeyStore, key: StoredInstallKey): void {
  keys.set(INSTALL_KEY_NAME, JSON.stringify(key));
}
...
export function getOrCreateInstallKey(keys: KeyStore, clientId: string): StoredInstallKey {
  const existing = peekInstallKey(keys, clientId);
  if (existing) return existing;
  const fresh = generateKeyPair(clientId);
  persistInstallKey(keys, fresh);
  return fresh;
}
 ...
 * this session cannot get one — an ordinary `Bearer` session, or a non-CIMD `--client` (DPoP is
 * only ever active for the CLI's own first-party CIMD identity — `dpop_mode` stays `'off'` for
 * every other materialized client server-side). Reads the SAME `KeyStore` entry
 * `getOrCreateInstallKey()` already generated/loaded at login (keyed by `session.client_id`) —
 * never a fresh key, or the server's `cnf.jkt` binding would not match.
 *
 * **`rakomi login --ci` sessions — Story rakomi-cli-ci-session-per-request-dpop-wiring.** That
 * flow's `client_id` is the sentinel `CI_FEDERATION_SESSION_CLIENT_ID` (not a CIMD URL), so it is
 * checked EXPLICITLY alongside `isCimdClientId`. `runLoginCi()` (`commands/login.ts`) hands
 * `exchangeOidcSubjectToken()` a key it can persist under this SAME `KeyStore` slot once the
 * exchange succeeds, so the token's `cnf.jkt` is bound to the key this function goes on to resolve
 * here, in a later, separate process. This key is durable for the remainder of the CI job's own
 * filesystem (the SAME `--no-keychain` file fallback that already durable-stores the job's access
 * token — `CI=true` selects it automatically, `resolveStores()`), never longer: an ephemeral
 * runner's disk is torn down with the job, so this is not a NEW class of at-rest exposure, it
 * matches the exposure the access token itself already has. Previously this function
 * unconditionally returned `undefined` for a `--ci` session (the key was generated fresh and
 * discarded on every call), so a SECOND authenticated CLI call in the same job always failed —
 * that gap is now resolved.
 */
export function resolveDpopKey(keys: KeyStore, session: StoredSession): StoredInstallKey | undefined {
  ...
  if (!isCimdClientId(session.client_id) && session.client_id !== CI_FEDERATION_SESSION_CLIENT_ID) return undefined;
