...
export function helpText(): string {
  return [
    ...
    '  tenants memberships      list tenants your account is a verified member of',
    '  tenants claim --ci       claim a temporary, self-expiring tenant under your own tenant',
    '    --tenant <id>          your own tenant to claim under (or run `rakomi use <tenant-id>` first)',
    '    --ttl-seconds <n>      how long the claim should live, in seconds (only for a freshly-created tenant)',
    '    --label <text>         optional label, stored and echoed back, never interpreted',
    '  tenants release --ci <tenant-id>  release a tenant claimed by `tenants claim`',
    '    --tenant <id>          the tenant it was claimed under (or run `rakomi use <tenant-id>` first)',
