...
- `PasskeySummary` gains an optional `sign_count_anomaly_at` field, set when the server flags a credential for a sign-count regression (a possible cloned authenticator). Additive only — existing callers are unaffected; the field is absent when no anomaly is on record.
