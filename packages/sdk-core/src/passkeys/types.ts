...
export interface PasskeySummary {
  ...
  created_at: string;
  /**
   * Present only when the server detected a sign-count regression on this credential (a possible
   * cloned authenticator). Absent when no anomaly is on record — treat it as absent-safe (optional)
   * rather than assuming every response carries it.
   */
  sign_count_anomaly_at?: string;
}
