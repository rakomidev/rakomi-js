...
/**
 * `resolveAuthorizationEndpoint()` could not determine the real hosted-login-UI
 * `authorization_endpoint` for a `baseUrl`: live discovery failed AND the
 * `api.`<->`accounts.` host-naming fallback does not apply. Never guessed —
 * a wrong endpoint would send a browser to a JSON API response instead of a login form.
 */
export const OAUTH_AUTHORIZATION_ENDPOINT_UNRESOLVED = (detail?: string) =>
  createError(
