...
export function createNativePasskeyAdapter(
   ...
   *
   * Known limitation of routing it this way: the core replaces the diagnosis with its own generic
   * "adapter returned a malformed credential", so the message naming the field and the rule — the
   * integrator's one chance to learn that their iOS module is emitting padded base64 — does NOT
   * reach the caller today. This package alone cannot restore it; the detail is only visible one
   * layer below, at the validator that first rejects the envelope.
   */
